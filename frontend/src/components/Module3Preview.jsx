...
import { Database, Search, Layers, Server, Zap, FileText, CheckCircle2, ArrowRight } from 'lucide-react';
...
export default function Module3Preview({ claims, onRetrievalComplete, onContinueToModule4 }) {
  ...
  const handleRunRetrieval = async () => {
    ...
    const docText = useCustom ? customKbText : PRESET_KNOWLEDGE_BASES[selectedKbIndex].text;
    const sourceDocs = [
      {
        id: "ground_truth_doc_1",
        text: docText
      }
    ];

    try {
      const data = await retrieveEvidence(claims, sourceDocs, 3);
    ...
    <div className="w-full max-w-7xl mx-auto space-y-8 animate-fadeIn">
      ...
      <div className="text-center py-6 glass-panel rounded-2xl border border-slate-800 shadow-2xl relative overflow-hidden">
        ...
        <h2 className="text-3xl font-extrabold text-white tracking-tight">
          Dense Vector Embedding & FAISS Search
        </h2>
        <p className="text-slate-400 text-sm max-w-2xl mx-auto mt-2">
          Encodes claims into 384-dimensional dense vectors using <code className="text-cyan-300 font-mono">all-MiniLM-L6-v2</code> and retrieves top-k matching evidence chunks from the knowledge base using FAISS inner-product search.
        </p>
      </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center space-x-3 text-xs">
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg font-bold transition-all ${
                    !useCustom ? 'bg-cyan-500/20 border border-cyan-500/50 text-cyan-300' : 'bg-slate-900 border border-slate-800 text-slate-400'
                  }`}
                >
                  Preset Corpora
                </button>
              ...
              {!useCustom ? (
                <select
                  ...
                  className="w-full bg-[#0B1120] border border-slate-700 rounded-xl px-3 py-2 text-xs font-semibold text-slate-200 focus:outline-none focus:border-cyan-500"
                >
                ...
                <textarea
                  ...
                  className="w-full bg-[#0B1120] border border-slate-700 rounded-xl p-3 text-xs text-slate-200 focus:outline-none focus:border-cyan-500 font-mono"
                />
              )}

              <div className="p-3 bg-[#0B1120] rounded-xl border border-slate-800/80 text-[11px] text-slate-400 space-y-1 font-mono">
                <div className="text-slate-300 font-bold">Document Content Preview:</div>
                <div className="line-clamp-3 italic text-slate-400">
                  "{useCustom ? customKbText : PRESET_KNOWLEDGE_BASES[selectedKbIndex].text}"
                </div>
              </div>
            </div>
          </div>
                ...
                claims.map((c, idx) => (
                  <div key={idx} className="p-3 rounded-xl bg-[#0B1120] border border-slate-800/80 space-y-1">
                    <div className="flex justify-between items-center text-[11px]">
                      ...
                      <span className="text-slate-500 font-mono">{c.category || 'Verifiable Claim'}</span>
                    </div>
                    <p className="text-xs text-slate-200">{c.text || c}</p>
                  </div>
                ))
              ) : (
            ...
            >
              {isRetrieving ? (
                <>
                  ...
                  <span>Embedding Claims & Querying FAISS...</span>
                </>
              ) : (
                <>
                  <Zap className="w-4 h-4" />
                  <span>Run Module 3 Vector Retrieval</span>
                </>
              )}
            </button>
          </div>
            ...
            {retrievalResults ? (
                        ...
                        item.evidence.map((ev, evIdx) => (
                          <div key={evIdx} className="p-3 rounded-lg bg-[#070C18] border border-slate-800 text-xs space-y-1.5">
                            <div className="flex justify-between items-center font-mono text-[11px]">
                              <span className="text-emerald-400 font-bold">Rank #{ev.rank} • {ev.chunk_id}</span>
                              <span className="text-cyan-300 font-bold bg-cyan-500/10 px-2 py-0.5 rounded border border-cyan-500/30">
                                Cosine Similarity: {ev.similarity_score}
                              </span>
                            </div>
                            <p className="text-slate-200 leading-relaxed font-sans text-xs">{ev.text}</p>
                          </div>
                        ))
                      ) : (
              ...
              <div className="h-80 flex flex-col items-center justify-center text-center space-y-3">
                ...
                <p className="text-xs text-slate-400 max-w-sm">
                  Select a reference knowledge base and click <strong>Run Module 3 Vector Retrieval</strong> to generate MiniLM dense embeddings and retrieve top-k evidence chunks.
                </p>
              </div>
            )}
          </div>
